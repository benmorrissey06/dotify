...
class GeminiHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def handle_analyze(self):
        try:
            ...
            api_key = os.environ.get('GEMINI_API_KEY', 'AIzaSyDflfGxaL6rSYy_cYccLL4NqAf4ymEUU98')
            
            if not api_key:
                self.send_error(500, "GEMINI_API_KEY environment variable not set")
                return
...
def run_server(port=8000):
    ...
    print(f"🔑 Gemini API Key: {'Set' if os.environ.get('GEMINI_API_KEY') else 'Using default'}")
